...
export const postSecureNumbers = async (
  ...
  await request.post('https://online.hl.co.uk/my-accounts/login-step-two', {
    body: JSON.stringify({      hl_vt: hl_vt,      'online-password-verification': password,      'secure-number[1]': secureNumber[secureNo1],      'secure-number[2]': secureNumber[secureNo2],      'secure-number[3]': secureNumber[secureNo3],      submit: 'Log+in',    }),
    json: {
      hl_vt: hl_vt,
